# Admin CRUD API Quick Test Script (With Authentication)
...
Write-Host "Admin CRUD API Quick Test (With Auth)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if ($token) {
    ...
    Write-Host "  Using no authentication (may fail if auth required)" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "  Note: If 403 Forbidden, check authentication" -ForegroundColor Yellow
    exit 1
}
...
try {
    ...
    $resolveResponse = Invoke-RestMethod -Uri "$resolveBaseUrl/resolve" -Method Post -Headers @{"Content-Type" = "application/json"} -Body $resolveBody
